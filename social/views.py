    ...
    def post(self, request, pk,*args, **kwargs):
        ...
        if form.is_valid():
            new_post =Post(
                ...
                body=orginal_post.body,
                author=orginal_post.author,
                created_on=orginal_post.created_on,
                shared_user= request.user,
                shared_on=timezone.now(),               
            )
            new_post.save() 
            
            for img in orginal_post.image.all():
                new_post.image.add(img)
            new_post.save()           
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if ThreadModel.objects.filter(user=request.user,receiver=receiver).exists() or ThreadModel.objects.filter(user=receiver,receiver=request.user).exists():
                thread = ThreadModel.objects.filter(user=request.user,receiver=request.user)[0]
